...
def headline
  table_to_hash('karaoke_detail_history')[0]['tostring']
end
...
describe 'Historyの編集/削除' , :js => true do
  ...
  it '編集' do
    find('#karaoke_detail_history').all('td')[7].find('img').click
  ...
  it '削除' do
    old_num = table_to_hash('karaoke_detail_history').length
    ...
    visit url
    find('#karaoke_detail_history').all('th')[6].click
    new_num = table_to_hash('karaoke_detail_history').length
